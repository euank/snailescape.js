...
;(function() {
  ...
  function Parser(opts) {
    ...
    this.doubleQuoteCharacters = opts.quoteCharacters || ['"'];
    this.singleQuoteCharacters = opts.rawQuoteCharacters || ["'"];
    this.spaceCharacters = opts.spaceCharacters || [" ", "\t"];
  }

  function findElement(els, chr) {
    for(var i=0; i < els.length; i++) {
      if(chr == els[i]) return chr;
    }
    return false;
  }
  ...
  Parser.prototype.parse = function(str) {
    ...
    var currentPart = '';
    var parts = [];
    // inSingleQuotes and inDoubleQuotes are overloaded to both indicate, by
    // being exactly false, that we are not currently in quotes, and to
    // indicate what quote we're in when we're in a quote.  E.g., if the caller
    // configured the quoteCharacters to be ['"', '/'], then inDoubleQuotes
    // will be set to '"' or '/' depending on which we are currently in.
    // Quoting is only ended by the same quote character that started it, so
    // this state is needed.  We do not support nesting quotes, so a stack of
    // quote characters we've seen is unneeded.
    ...
    for(i = 0; i < str.length; i++) {
      ...
      if(inDoubleQuotes !== false) {
        ...
        if(doubleQuoteChar !== false) {
            inDoubleQuotes = doubleQuoteChar;
            continue;
        }

        var singleQuoteChar = findElement(self.singleQuoteCharacters, str[i]);
        if(singleQuoteChar !== false) {
            inSingleQuotes = singleQuoteChar;
            continue;
        }

        // Also a valid time to start a new part when we're not in double
        // quotes; outside of double quotes, space chars have ~special meaning~
        // of splitting into parts
        var spaceChar = findElement(self.spaceCharacters, str[i]);
        if(spaceChar !== false) {
          if(currentPart !== "") {
            parts.push(currentPart);
            currentPart = "";
      ...
      // Characters; we're either in double quotes or in unqouted text; all escapes apply equally.
      if(str[i] === '\\') {
        if(i >= str.length - 1) {
        ...
        var doubleQuoteChar = findElement(self.doubleQuoteCharacters, str[i]);
        if(doubleQuoteChar !== false) {
          currentPart += doubleQuoteChar;
          continue;
        }
        var singleQuoteChar = findElement(self.singleQuoteCharacters, str[i]);
        if(singleQuoteChar !== false) {
          currentPart += singleQuoteChar;
          continue;
        }
        var spaceChar = findElement(self.spaceCharacters, str[i]);
        if(spaceChar !== false) {
          currentPart += spaceChar;
          continue;
        }
        // Here we go, backslash escapes
  ...
  // which will then convert back into the array when parsed using default
  // configurations.
